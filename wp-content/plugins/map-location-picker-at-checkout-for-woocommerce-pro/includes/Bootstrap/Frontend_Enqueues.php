...
class Frontend_Enqueues {
	...
	public function enqueue_styles() {
		wp_enqueue_style( $this->plugin_name, LPAC_PLUGIN_ASSETS_PATH_URL . 'public/css/lpac-public.css', array(), $this->version . $this->maybe_burst_cache, 'all' );
	}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, LPAC_PLUGIN_ASSETS_PATH_URL . 'public/js/' . $path . 'lpac-public.js', array( 'jquery', 'wp-util' ), $this->version . $this->maybe_burst_cache, false );
		...
		wp_register_script( $this->plugin_name . '-google-maps-js', $map_resource, array(), $this->version . $this->maybe_burst_cache, false );
		...
		if ( is_wc_endpoint_url( 'view-order' ) || is_wc_endpoint_url( 'order-received' ) || is_checkout() ) {
			...
			*/
			wp_enqueue_script( $this->plugin_name . '-base-map', LPAC_PLUGIN_ASSETS_PATH_URL . 'public/js/maps/' . $path . 'base-map.js', array( $this->plugin_name . '-google-maps-js' ), $this->version . $this->maybe_burst_cache, true );

			/**
			 * Load order received page map
			 */
			if ( is_wc_endpoint_url( 'order-received' ) ) {
				wp_enqueue_script( $this->plugin_name . '-order-received-map', LPAC_PLUGIN_ASSETS_PATH_URL . 'public/js/maps/' . $path . 'order-received-map.js', array( $this->plugin_name . '-base-map' ), $this->version . $this->maybe_burst_cache, true );
			}
			...
			if ( is_wc_endpoint_url( 'view-order' ) ) {
				wp_enqueue_script( $this->plugin_name . '-order-details-map', LPAC_PLUGIN_ASSETS_PATH_URL . 'public/js/maps/' . $path . 'order-details-map.js', array( $this->plugin_name . '-base-map' ), $this->version . $this->maybe_burst_cache, true );
			}
			...
			if ( is_checkout() && ! is_wc_endpoint_url( 'order-received' ) ) {
				 ...
				 */
				wp_enqueue_script( $this->plugin_name . '-checkout-page-map', LPAC_PLUGIN_ASSETS_PATH_URL . 'public/js/maps/' . $path . 'checkout-page-map.js', array( $this->plugin_name . '-base-map', 'wp-util' ), $this->version . $this->maybe_burst_cache, true );

				if ( lpac_fs()->can_use_premium_code__premium_only() ) {
					wp_enqueue_script( $this->plugin_name . '-checkout-page-map-pro', LPAC_PLUGIN_ASSETS_PRO_PATH_URL . 'public/js/maps/' . $path . 'checkout-page-map-pro.js', array( $this->plugin_name . '-base-map', 'wp-util' ), $this->version . $this->maybe_burst_cache, true );
				}
			}
		}
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			if ( is_account_page() ) {
				wp_enqueue_script( $this->plugin_name . '-public-pro', LPAC_PLUGIN_ASSETS_PRO_PATH_URL . 'public/js/lpac-public-pro.js', array( 'jquery', 'wp-util' ), $this->version . $this->maybe_burst_cache, false );
				wp_enqueue_style( $this->plugin_name . '-public-pro', LPAC_PLUGIN_ASSETS_PRO_PATH_URL . 'public/css/lpac-my-account-pro.css', array(), $this->version . $this->maybe_burst_cache );
			}
			wp_enqueue_style( $this->plugin_name . '-public-pro', LPAC_PLUGIN_ASSETS_PRO_PATH_URL . 'public/css/lpac-public-pro.css', array(), $this->version . $this->maybe_burst_cache );
			// Register our map builder script to later be enqueued inside our custom maps shortcode.
			wp_register_script( $this->plugin_name . '-map-builder', LPAC_PLUGIN_ASSETS_PRO_PATH_URL . 'public/js/' . $path . 'kikote-map-builder.js', array( 'jquery', $this->plugin_name . '-google-maps-js' ), $this->version . $this->maybe_burst_cache, false );
		}
