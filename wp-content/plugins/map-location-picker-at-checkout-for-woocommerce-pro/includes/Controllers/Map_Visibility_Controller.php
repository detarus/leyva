...
class Map_Visibility_Controller {
	...
	public function checkout_map_rules_order_ajax_handler() {
		...
		if ( ! wp_verify_nonce( $nonce, 'lpac_map_visibility_rules_order' ) ) {
			wp_send_json_error( 'Nonce check failed', 403 );
		}

		if ( empty( $items ) ) {
			wp_send_json_error( 'Kikote: Handler found no items in rulesOrder request.', 500 );
		}
	...
	public static function get_map_visibility_rules() {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			...
			$rules_pro = array(
				'shipping_zones' => __( 'Shipping Zones', 'map-location-picker-at-checkout-for-woocommerce' ),
				'cart_total'     => __( 'Cart Subtotal', 'map-location-picker-at-checkout-for-woocommerce' ),
			);

			$rules = array_merge( $rules, $rules_pro );
	...
	private static function hide_map_by_shipping_method() {
		...
		if ( empty( $checkout_shipping_method ) ) { // In these instances, map will also show if no shipping methods are available for customer
			return true;
		}
		...
		$present = in_array( $instance_id, $shipping_methods, true );
	...
	private static function shipping_zones( $show ) {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			...
			/* Get the shipping zone IDs selected by the admin. */
			$selected_shipping_zones_ids = get_option( 'lpac_wc_shipping_zones', array() );

			if ( ! empty( $selected_shipping_zones_ids ) ) {
				$show = \Lpac\Pro\Helpers\Functions::lpac_should_show_shipping_zone( $selected_shipping_zones_ids );
			}
		}
	...
	private static function cart_total( $show ) {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			...
			/* Hide or show map based on cart amount. */
			if ( ! empty( $min_cart_amount ) || ! empty( $max_cart_amount ) ) {
				$show = \Lpac\Pro\Helpers\Functions::show_map_by_cart_amount();
			}
		}
