...
use Lpac\Pro\Models\Plugin_Settings\DisplaySettings;
...
class Functions extends Lite_Functions_Helpers {
	 ...
	 */
	public static function exposePremiumJS() {

		$output           = self::getPremiumJSSettings();
...
JAVASCRIPT;
	 ...
	 * @since 1.7.0
	 */
	public static function getPremiumJSSettings() {

		$google_map_id = '';

		if ( is_checkout() && ! is_wc_endpoint_url( 'order-received' ) ) {
			$google_map_id = DisplaySettings::getCheckoutPageMapId();
		}

		if ( is_wc_endpoint_url( 'view-order' ) ) {
			$google_map_id = DisplaySettings::getViewOrderPageMapId();
		}

		if ( is_wc_endpoint_url( 'order-received' ) ) {
			$google_map_id = DisplaySettings::getOrderReceivedPageMapId();
		}

		if ( function_exists( 'get_current_screen' ) ) {
			$screen = get_current_screen()->id;
			if ( $screen === 'woocommerce_page_wc-settings' || $screen === 'woocommerce_page_wc-orders' || $screen === 'shop_order' ) {
				$google_map_id = DisplaySettings::getAdminViewOrderMapId();
			}
		};
	 ...
	 *
	 * This helps prevent customers from being able to checkout using an empty(as in no price) shipping method that was attached to the plugin shipping workflows.
	 *
	 * @param array  $rates
	...
	public static function removeEmptyKikoteShippingMethodsFromRates( array $rates, string $workflow ): array {
		...
		foreach ( $selected_shipping_methods_ids as $selected_shipping_methods_id ) {
			$selected_shipping_methods_id = (int) $selected_shipping_methods_id;
			/**
			 * @var \WC_Shipping_Rate $rate_class
			 */
			foreach ( $rates as $rate_key => $rate_class ) {

				if ( $rate_class->get_instance_id() === $selected_shipping_methods_id ) {
