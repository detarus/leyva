...
class Location_Details {
	 ...
	 * @return void
	 */
	public function saveCustomerRegion( int $order_id, array $data ) : void {
		...
		$order->add_meta_data( 'lpac_customer_region', $customer_region );
		$order->save();
	}
	...
	public function save_customer_distance_data( int $order_id, array $data ) : void {
		...
		$order = wc_get_order( $order_id );
		$order->add_meta_data( 'lpac_customer_distance', $customer_distance );
		$order->add_meta_data( 'lpac_customer_distance_unit', $customer_distance_unit );
		$order->add_meta_data( 'lpac_customer_distance_duration', $customer_distance_duration );
		$order->save();
	}
