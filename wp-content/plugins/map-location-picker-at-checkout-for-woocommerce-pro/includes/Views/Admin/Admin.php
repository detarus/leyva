...
/**
 * Class responsible for creating various markup of the plugin on the WP admin pages.
 *
...
class Admin {
	...
	public function lpac_display_lpac_admin_order_meta( $order ) {
		...
		if ( ! empty( $places_autocomplete_used ) ) {
			$places_autocomplete_used_text = sprintf( esc_html( __( 'It looks like this customer used the Places Autocomplete feature. The coordinates on the map might be an approximation. %s' ) ), "<a href='https://lpacwp.com/docs/places-autocomplete-feature/#accuracy-of-places-autocomplete' target='_blank'> $learn_more </a>" );
		}
		...
		<div id='lpac-admin-order-meta'>
			<div class="lpac-admin-order-meta-location">
				...
				<p style="font-size: 12px"><?php echo $places_autocomplete_used_text; ?></p>
			</div>
		<?php
	 ...
	 * @since    1.1.2
	 */
	public function createCustomOrderDetailsMetabox() {
		...
		if ( OrderUtil::is_order( $order_id, wc_get_order_types() ) === false ) {
			return;
		}
		...
		add_meta_box( 'lpac_delivery_map_metabox', __( 'Location', 'map-location-picker-at-checkout-for-woocommerce' ), array( $this, 'output_custom_order_details_metabox' ), $screen, 'normal', 'high' );
	}
	...
	public function output_custom_order_details_metabox() {
		...
		$order = wc_get_order(); // Gets order ID from global scope
...
JAVASCRIPT;
	...
	public function create_custom_wc_settings_info_text( $value ) {
		...
		$text            = $value['text'];
		$css             = $value['css'];
		$option_name_css = $value['option_name_css'] ?? '';

		?>
			<tr valign='top' class="<?php echo esc_attr( $row_class ); ?>">
			<th scope='row' style="<?php echo esc_attr( $option_name_css ); ?>" class="titledesc"><?php echo esc_html( $name ); ?></th>
			<td>
	...
	public function create_custom_wc_settings_repeater( $value ) {
		...
		$multiple_select               = $value['select_multiple'] ?? false; // The options to populate the select field dropdown with
				...
				foreach ( $fields as $field_name ) {
					...
					switch ( $type ) {
						case 'select':
							...
							$multiple = ( $multiple_select ) ? "class='wc-enhanced-select select2-field' multiple='multiple'" : '';

							if ( (bool) $multiple ) {

								$select_shipping_methods = $current_saved_settings[ $select_element_id ] ?? array();
								...
								foreach ( $select_field_dropdown_options as $key => $option_details ) {
									...
									if ( in_array( $item_id, $select_shipping_methods, false ) ) { // In array should be false here because $item_id would be a string.
										$options .= "<option value='" . esc_attr( $item_id ) . "' selected='selected'>" . esc_html( $item_value ) . '</option>';
									} else {
							...
							$hold_inputs .= "
									<td>
										<select $multiple name='" . esc_attr( $select_element_id ) . "' $fields_disabled>
			...
			foreach ( $table_columns as $key => $value ) {
				...
				switch ( $type ) {
					case 'select':
						...
						$hold_inputs .= "
						<td>
							<select $multiple name='$select_element_id' $fields_disabled>
	...
	public function create_custom_wc_settings_upsell_banner( $value ) {
		...
		$signup_text = sprintf( esc_html__( 'Custom Maps, Custom Marker Icons, Saved Addresses, More Visibility Rules, Cost by Region, Cost by Distance, Cost by Store Location, Multi-Store Distance Pricing, Export Order Locations & More. %s Get the most out of Kikote with the PRO version.', 'map-location-picker-at-checkout-for-woocommerce' ), '<br/><br/>' );
		/* translators: 1: Dashicons outbound link icon */
	 ...
	 * @return mixed
	 */
	public function addMapBtnAdminListColumn( $columns ) {
		$columns['lpac_location'] = __( 'Location', 'map-location-picker-at-checkout-for-woocommerce' );
		return $columns;
	}

	/**
	 * Add map link to our custom column.
	 *
	 * @param object $order
	 * @return void
	 * @since 1.9.0
	 */
	private function prepareLocationBtnContent( object $order ) {
	 ...
	 * @since 1.9.0
	 */
	private function prepareStoreLocationContent( object $order ) {
	...
	public function addKikoteColumns( array $columns ): array {
		...
		$columns['lpac_location'] = __( 'Location', 'map-location-picker-at-checkout-for-woocommerce' );
		$store_locations_model    = new \Lpac\Models\Plugin_Settings\Store_Locations();
		...
		if ( ( get_option( 'lpac_enable_shipping_cost_by_distance_feature' ) === 'yes' && get_option( 'lpac_enable_cost_by_store_distance' ) === 'yes' ) || get_option( 'lpac_enable_cost_by_store_location' ) === 'yes' || $store_locations_model::showStoreSelectorInCheckoutEnabled() ) {
	 ...
	 * @since 1.9.0
	 */
	public function addKikoteColumnsContent( string $column, $order ) {
		...
		if ( $column === 'lpac_location' ) {
			$this->prepareLocationBtnContent( $order );
		}
