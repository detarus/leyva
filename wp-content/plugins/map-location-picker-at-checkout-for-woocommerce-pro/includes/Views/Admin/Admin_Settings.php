...
class Admin_Settings extends \WC_Settings_Page {
	...
	public function create_plugin_settings_sections() {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			$sections = array_merge(
				array_slice( $sections, 0, 5, true ),
				array( 'map_builder' => __( 'Map Builder', 'map-location-picker-at-checkout-for-woocommerce' ) ),
				array_slice( $sections, 5, null, true )
			);
		}
	...
	public static function create_plugin_settings_banner() {
		...
		if ( empty( get_option( 'lpac_google_maps_api_key', '' ) ) ) {
			...
			$no_api_key .= "<a href='https://lpacwp.com/docs/getting-your-google-maps-api-key/?utm_source=banner&utm_medium=lpacdashboard&utm_campaign=freedocs' target='_blank'>$here</a>";
		} else {
			$no_api_key = '';
		}
...
HTML;
	...
	public function create_general_setting_fields() {
		...
		$lpac_settings[] = array(
			'name' => __( 'General Settings', 'map-location-picker-at-checkout-for-woocommerce' ),
		...
		*/
		if ( empty( $plugin_enabled ) ) {
			...
			$lpac_settings[] = array(
				...
				'desc_tip' => __( 'Enable the plugin and its features.', 'map-location-picker-at-checkout-for-woocommerce' ),
				'id'       => 'lpac_enabled',
				'type'     => 'checkbox',
				'css'      => 'max-width:80px;',
				// 'cbvalue' => 'yes',
		...
		$lpac_settings[] = array(
			...
			'desc'              => __( 'Enter the API key you copied from the Google Cloud Console. Without the Google Maps API key the plugin will not work. <a href="https://lpacwp.com/docs/getting-your-google-maps-api-key/?utm_source=generaltab&utm_medium=lpacdashboard&utm_campaign=freedocs" target="blank">Learn More <span style="text-decoration: none" class="dashicons dashicons-external"></span></a>', 'map-location-picker-at-checkout-for-woocommerce' ),
		...
		$lpac_settings[] = array(
			...
			/* translators: 1: Learn more link 2: HTML Break element */
		...
		$lpac_settings[] = array(
			...
			/* translators: 1: Learn more link*/
			'desc_tip' => sprintf( __( 'This option displays a map view on the order details page in the customer account. %s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://lpacwp.com/docs/general-settings/?utm_source=generaltab&utm_medium=lpacdashboard&utm_campaign=freedocs#show-map-on-view-order-page' target='blank'> " . self::$learn_more . ' </a>' ),
	...
	private function create_dummy_general_settings_fields() {
		...
		$dummy_general_pro_settings[] = array(
			...
			'desc'  => sprintf( __( 'Allow customers to save different addresses for later use. %s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://lpacwp.com/docs/general-settings/?utm_source=generaltab&utm_medium=lpacdashboard&utm_campaign=prodocs#saved-addresses' target='blank'>" . self::$learn_more . '</a>' ),
			'type'  => 'hr',
			'id'    => 'test',
		);
	...
	private function create_display_settings_fields() {
		...
		$lpac_settings[] = array(
			'name' => __( 'Display Settings', 'map-location-picker-at-checkout-for-woocommerce' ),
	...
	private function create_store_locations_settings_fields() {
		...
		$store_locations_table_columns = array(
		...
		$adjusted_shipping_methods_array = array(
			array(
				'shipping_method_id'   => 0,
				'shipping_method_name' => __( 'Available in PRO', 'map-location-picker-at-checkout-for-woocommerce' ),
			),
		);

		if ( lpac_fs()->can_use_premium_code__premium_only() ) {

			$all_shipping_methods            = FunctionsHelper::getAvailableShippingMethods();
			$adjusted_shipping_methods_array = array();
			foreach ( $all_shipping_methods as $shipping_method_id => $shipping_method_name ) {
				$adjusted_shipping_methods_array[] = array(
		...
		$lpac_settings[] = array(
			'name'                          => __( 'Store Locations', 'map-location-picker-at-checkout-for-woocommerce' ),
			'desc'                          => sprintf( __( 'Create your different store locations. A "store" in this context simply means the locations where you do business. Separate latitude and longitude coordinates with a comma. You can use a website like %1$sthis one%2$s to get the GPS coordinates for an address. %3$s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://gps-coordinates.org/' target='_blank'>", "<span style='text-decoration: none' class='dashicons dashicons-external'></span></a>", "<a href='https://lpacwp.com/docs/store-locations/?utm_source=storelocationstab&utm_medium=lpacdashboard&utm_campaign=freedocs' target='blank'>" . self::$learn_more . '</a>' ),
			'id'                            => 'lpac_store_locations',
			'type'                          => 'repeater',
			'current_saved_settings'        => Store_Locations::getStoreLocations(),
			'entity_name'                   => __( 'location', 'map-location-picker-at-checkout-for-woocommerce' ),
			'id_field'                      => 'store_location_id', // The field that will be used as the unique identifier for the entry
			'table_columns'                 => $store_locations_table_columns,
			'select_field_dropdown_options' => $adjusted_shipping_methods_array,
			'select_multiple'               => true,
			'option_element_id'             => 'shipping_method_id',
			'option_element_value'          => 'shipping_method_name',
			'select_element_id'             => 'store_locations_shipping_method_select',
		);
		...
		$lpac_settings[] = array(
			...
			'id'        => 'lpac_store_selector_shortcode',
			'text'      => sprintf( esc_html__( 'You can use the shortcode: %1$s[kikote_store_selector]%2$s anywhere on your website to allow customers (including guests) to set their preferred store to order from. The selected store will automatically be chosen at checkout.  %3$s', 'map-location-picker-at-checkout-for-woocommerce' ), '<code>', '</code>', '<a href="https://lpacwp.com/docs/store-locations/?utm_source=storelocationstab&utm_medium=lpacdashboard&utm_campaign=freedocs#shortcode" target="_blank">' . self::$learn_more . '</a>' ),
			'type'      => 'info_text',
			'is_option' => false,
		);
	...
	private function create_dummy_display_settings_fields() {
		...
		$lpac_dummy_display_pro_settings[] = array(
			...
			'class' => 'dashicons-before dashicons-lock premium-dummy-subsection',
			'type'  => 'hr',
			'desc'  => sprintf( __( 'Set the Map ID for the respective Maps. You can create a custom map for each entry. %s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://lpacwp.com/docs/display-settings/?utm_source=displaytab&utm_medium=lpacdashboard&utm_campaign=prodocs#map-id' target='blank'>" . self::$learn_more . '</a>' ),
		);
	...
	private function create_visibility_settings_fields() {
		...
		$lpac_settings[] = array(
			'name' => __( 'Map Visibility Rules', 'map-location-picker-at-checkout-for-woocommerce' ),
		...
		$lpac_settings[] = array(
			...
			'class'   => 'wc-enhanced-select',
			'desc'    => sprintf( __( 'Hide the map when any of these shipping methods are chosen by the user. %s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://lpacwp.com/docs/visibility-rules/?utm_source=visibilityrulestab&utm_medium=lpacdashboard&utm_campaign=freedocs#hide-map-for-shipping-methods' target='blank'>" . self::$learn_more . '</a>' ),
			'id'      => 'lpac_wc_shipping_methods',
			'type'    => 'multiselect',
			'options' => FunctionsHelper::getAvailableShippingMethods(),
			'css'     => 'min-width:300px;height: 100px',
		);
		...
		$lpac_settings[] = array(
			...
			'type'    => 'multiselect',
			'options' => FunctionsHelper::lpac_get_available_shipping_classes(),
			'css'     => 'min-width:300px;height: 100px',
		);
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			$lpac_settings = \Lpac\Pro\Views\Admin\Admin_Settings::create_pro_visibility_settings_fields( $lpac_settings );
		}
	...
	private function create_dummy_shipping_setting_fields() {
		...
		$lpac_dummy_pro_settings[] = array(
			...
			'select_field_dropdown_options' => array(
				array(
					'name' => '',
				),
		...
		if ( empty( $saved_cost_by_store_distance_delivery_prices ) ) {
			...
			$saved_cost_by_store_distance_delivery_prices = array(
				array(
		...
		if ( empty( $saved_cost_by_store_location_delivery_prices ) ) {
			...
			$saved_cost_by_store_location_delivery_prices = array(
				array(
					'store_location_id_select' => 'store_location_0',
					'store_price_text'         => '2',
				),
	...
	private function create_tools_setting_fields() {
		...
		$lpac_settings[] = array(
			'name' => __( 'Tools', 'map-location-picker-at-checkout-for-woocommerce' ),
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			$lpac_settings[] = array(
				'name'     => __( 'Show Shipping Distance', 'map-location-picker-at-checkout-for-woocommerce' ),
				'desc'     => __( 'Yes', 'map-location-picker-at-checkout-for-woocommerce' ),
				'desc_tip' => __( 'Enabling this option will show the current distance between the origin and the selected location on the map as calculated by the Google API.', 'map-location-picker-at-checkout-for-woocommerce' ),
				'id'       => 'lpac_debug_shipping_distance',
				'type'     => 'checkbox',
			);
		}
	...
	private function create_more_page() : array {
		...
		$lpac_settings[] = array(
			'type'      => 'lpac_image',
			'src'       => LPAC_PLUGIN_ASSETS_PATH_URL . 'admin/img/printus.png',
			'desc'      => __( 'Automatically print order invoices, receipts, package slips and labels to your local printer.', 'map-location-picker-at-checkout-for-woocommerce' ) . " <a href='https://wordpress.org/plugins/printus-cloud-printing-for-woocommerce/' target='_blank'>" . self::$learn_more . '</a>',
			'height'    => '280px',
			'url'       => 'https://wordpress.org/plugins/delivery-and-pickup-scheduling-for-woocommerce/',
			'is_option' => false,
		);
	...
	private function output_map_visibility_rules_order() {
		...
		if ( count( $rules ) !== count( $default_visibility_rules ) ) {
			...
			foreach ( $new_rules as $key => $value ) {
				echo '#' . esc_html( $key ) . '{ background: #FBFF12 !important; }';
			}
			echo '</style>';

		}
		/* translators: 1: Learn more link*/
		$info_text = sprintf( esc_html__( 'Use the table below to arrange the map visibility rules by dragging and dropping. The last rule in the the table decides the final visibility state of the map. %s', 'map-location-picker-at-checkout-for-woocommerce' ), "<a href='https://lpacwp.com/docs/visibility-rules/?utm_source=visibilityrulestab&utm_medium=lpacdashboard&utm_campaign=freedocs#rules-order' target='blank'>" . self::$learn_more . '</a>' );
		?>

		<div>
			<table id="lpac-rules" class='wc-shipping-zones'>
				<h4><?php echo wp_kses( $info_text, '<a>' ); ?></h4>
				<p id="lpac-rules-saving" style="display: none; font-weight: 700;"><?php esc_html_e( 'Saving...' ); ?></p>
				<p id="lpac-rules-saving-success" style="color: #008000; display: none; font-weight: 700;"><?php esc_html_e( 'Saved successfully' ); ?></p>
				<p id="lpac-rules-saving-failed" style="color: #d63638; display: none; font-weight: 700;"><?php esc_html_e( 'An error occurred while saving rules, please try again...' ); ?></p>
				<?php wp_nonce_field( 'lpac_map_visibility_rules_order', 'lpac-rules-order-nonce' ); ?>
				<thead>
				<tr>
					<th><span class="woocommerce-help-tip"></span></th>
					<th><?php esc_html_e( 'Rules Order', 'map-location-picker-at-checkout-for-woocommerce' ); ?></th>
				</tr>
			</thead>

			<tbody>
			
			<?php foreach ( $rules as $rule => $rule_name ) : ?>
			<tr data-id="<?php echo esc_attr( $rule ); ?>">
			<td width="1%" class="wc-shipping-zone-sort ui-sortable-handle"></td>
			<td id="<?php echo esc_attr( $rule ); ?>">
	...
	public function create_plugin_settings_fields() {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			$Pro_Admin_Settings = new \Lpac\Pro\Views\Admin\Admin_Settings();
		}
		...
		if ( empty( $current_section ) || $current_section === 'general' ) {
			...
			if ( lpac_fs()->can_use_premium_code__premium_only() ) {
				$pro_general_settings = $Pro_Admin_Settings->create_general_settings_fields();
				$lpac_settings        = array_merge( $lpac_settings, $pro_general_settings );
			}
		...
		if ( $current_section === 'display' ) {
			...
			if ( lpac_fs()->can_use_premium_code__premium_only() ) {
				$pro_display_settings = $Pro_Admin_Settings->create_display_settings_fields();
				$lpac_settings        = array_merge( $lpac_settings, $pro_display_settings );
			}
		...
		if ( $current_section === 'export' ) {
			...
			if ( lpac_fs()->can_use_premium_code__premium_only() ) {
				$lpac_pro_export_settings = $Pro_Admin_Settings->create_export_settings_fields();
				$lpac_settings            = array_merge( $lpac_settings, $lpac_pro_export_settings );

				// Setup our map view
				$admin_settings_controller = new \Lpac\Pro\Controllers\AdminSettingsController();
				$admin_settings_controller->setupExportSettingsMapOptions();
			}
		}
		...
		if ( $current_section === 'shipping' ) {
			...
			if ( lpac_fs()->can_use_premium_code__premium_only() ) {
				$lpac_pro_shipping_settings = $Pro_Admin_Settings::create_shipping_settings_fields();
				$lpac_settings              = array_merge( $lpac_settings, $lpac_pro_shipping_settings );

				// Setup our map view
				$admin_settings_controller = new \Lpac\Pro\Controllers\AdminSettingsController();
				$admin_settings_controller->setupShippingSettingsMapOptions();
			}
		}
