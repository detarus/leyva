...
class Frontend {
	...
	private function setup_global_js_vars( $additional = array() ) {
		...
		$map_options['places_address_components_mappings'] = apply_filters(
			'places_address_components_mappings',
			array(
				'country'     => 'country',
				'townCity1'   => 'locality',
...
JAVASCRIPT;
	 ...
	 * @since 1.6.0
	 * @return bool
	 * @deprecated Use setting from Shipping_Settings PRO class.
	 */
	private function get_cost_by_distance_setting() {
		...
		return filter_var( $enable_cost_by_distance, FILTER_VALIDATE_BOOLEAN );
	}
	...
	private function get_cost_by_store_location_setting() {
		...
		return filter_var( $enable_cost_by_store_location, FILTER_VALIDATE_BOOLEAN );
	}
	...
	/**
	 * Create Saved Addresses fields.
	 *
	 * Code block present only in premium
	 *
	 * @return void
	 */
	private function maybe_create_saved_addresses_fields__premium_only() :void {
		...
		/*
		* Create the save address checkbox and input area
		*/
		$save_address_checkbox_label = __( 'Save the Shipping Address for the future?', 'map-location-picker-at-checkout-for-woocommerce' );
		$save_address_checkbox_label = apply_filters( 'lpac_save_address_for_future_text', $save_address_checkbox_label );

		$enter_address_name_label = __( 'Enter Address Name', 'map-location-picker-at-checkout-for-woocommerce' );
		$enter_address_name_label = apply_filters( 'lpac_enter_address_name_label', $enter_address_name_label );

		woocommerce_form_field(
			'lpac_save_address_checkbox',
			array(
				'type'     => 'checkbox',
				'label'    => $save_address_checkbox_label,
				'required' => false,
		...
		woocommerce_form_field(
			'lpac_saved_address_name',
			array(
				'type'     => 'text',
				'label'    => $enter_address_name_label,
				'required' => false,
				'class'    => array( 'form-row-wide', 'hidden', 'fc-skip-hide-optional-field' ), // fc-skip-hide-optional-field - Fluid Checkout support: Prevent Fluid Checkout from hiding field with expandable "+" button.
				'priority' => 1,
			),
			''
		);
	...
	/**
	 * Create cost by store distance dropdown field.
	 *
	 * Code block present only in premium
	 *
	 * @since 1.6.0
	 * @return void
	 */
	private function maybe_create_cost_by_store_distance_fields__premium_only(): void {

		$enable_cost_by_store_distance = $this->get_cost_by_store_distance_setting();
		$enable_cost_by_distance       = $this->get_cost_by_distance_setting();
		$store_distance_pricing        = get_option( 'lpac_cost_by_store_distance_delivery_prices', array() );

		// $enable_cost_by_distance needs to be true to proceed because this setting needs to be enabled or else the cost by store distance feature wouldn't run
		if ( $enable_cost_by_distance && $enable_cost_by_store_distance && ! empty( $store_distance_pricing ) ) {

			$current_set_locations = array_column( $store_distance_pricing, 'store_location_id_select' );
			$store_locations       = $this->get_normalized_store_locations();

			// Remove location from drop down if store owner hasn't set a shipping cost for it.
			foreach ( $store_locations as $location_id => $location_name ) {
				if ( ! in_array( $location_id, $current_set_locations ) ) {
					unset( $store_locations[ $location_id ] );
				}
			}

			$store_locations = array_merge( array( '' => '--' . __( 'Please choose an option', 'map-location-picker-at-checkout-for-woocommerce' ) . '--' ), $store_locations );

			woocommerce_form_field(
				'lpac_order__origin_store',
				array(
					'type'     => 'select',
	...
	/**
	 * Create cost by store location dropdown field.
	 *
	 * Code block present only in premium
	 *
	 * @since 1.6.0
	 * @return void
	 */
	private function maybe_create_cost_by_store_location_fields__premium_only(): void {

		$enable_cost_by_store_location = $this->get_cost_by_store_location_setting();
		$store_location_pricing        = get_option( 'lpac_cost_by_store_location_delivery_prices', array() );

		if ( $enable_cost_by_store_location && ! empty( $store_location_pricing ) ) {

			$current_set_locations = array_column( $store_location_pricing, 'store_location_id_select' );
			$store_locations       = $this->get_normalized_store_locations();
	...
	private function maybe_create_store_location_selector_fields(): void {
		 ...
		 */
		if ( ( $this->get_cost_by_distance_setting() === false || $this->get_cost_by_store_distance_setting() === false ) &&
			 $this->get_cost_by_store_location_setting() === false &&
			 Store_Locations::showStoreSelectorInCheckoutEnabled() === true
			 ) {

			$store_locations = array_merge( array( '' => '--' . __( 'Please choose an option', 'map-location-picker-at-checkout-for-woocommerce' ) . '--' ), $this->get_normalized_store_locations() );
	...
	private function create_lpac_checkout_fields() : void {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			$this->maybe_create_saved_addresses_fields__premium_only();
			$this->maybe_create_cost_by_store_distance_fields__premium_only();
			$this->maybe_create_cost_by_store_location_fields__premium_only();
		}
		...
		woocommerce_form_field(
			...
			array(
				...
				'required'          => false,
				'class'             => ( LPAC_DEBUG ) ? array( 'form-row-wide', 'fc-skip-hide-optional-field' ) : array( 'form-row-wide', 'hidden' ), // fc-skip-hide-optional-field - Fluid Checkout support: Prevent Fluid Checkout from hiding field with expandable "+" button.
				'custom_attributes' => array( 'readonly' => true ),
				'default'           => 0,
			)
		);
	 ...
	 * @since    1.0.0
	 */
	public function outputCheckoutMap() {
		...
		if ( lpac_fs()->can_use_premium_code__premium_only() ) {
			if ( get_option( 'lpac_places_autocomplete_searchbox_in_map' ) === 'yes' && get_option( 'lpac_enable_places_autocomplete', 'no' ) === 'yes' ) {
					$styles = apply_filters( 'kikote_map_search_box_styles', 'margin-top: 10px; display: none; padding: 10px; width: 40%' );
				?>
					<input id="lpac-map-searchbox" style='<?php echo esc_attr( $styles ); ?>' size='30' type="text" placeholder="<?php echo esc_html__( 'Search...', 'map-location-picker-at-checkout-for-woocommerce' ); ?>"/>
				<?php
			}
		}
		?>
		<div style='display: <?php echo esc_attr( $display ); ?>' id='lpac-map-container' class='woocommerce-shipping-fields__field-wrapper'>
			<?php do_action( 'lpac_before_checkout_map', '', $user_id ); ?>
			<div class='lpac-map'></div>
			<?php do_action( 'lpac_after_checkout_map', '', $user_id ); ?>
			<div class='lpac-map-controls'>
			<?php do_action( 'lpac_before_checkout_map_controls', '', $user_id ); ?>
			<div id='lpac-map-instructions'> <?php echo wp_kses_post( $instuctions_text ); ?></div>
			<?php do_action( 'lpac_before_detect_location_btn', '', $user_id ); ?>
			<div id='lpac-find-location-btn-wrapper'><button id='lpac-find-location-btn' class='button btn' type='button'><?php echo esc_html( $lpac_find_location_btn_text ); ?></button></div>
			<?php do_action( 'lpac_after_detect_location_btn', '', $user_id ); ?>
			<div id='lpac-saved-addresses'>
				<?php do_action( 'lpac_before_saved_addresses', '', $user_id ); ?>
				<ul>
					<?php do_action( 'lpac_saved_addresses', '', $user_id ); ?>
				</ul>
				<?php do_action( 'lpac_after_saved_addresses', '', $user_id ); ?>
			</div>
			<?php do_action( 'lpac_after_checkout_map_controls', '', $user_id ); ?>
			</div>
		</div>
		<?php
	...
	public function output_origin_store_name() {
		...
		if ( Store_Locations::showStoreSelectorInCheckoutEnabled() === false &&
			 $this->get_cost_by_store_distance_setting() === false &&
			 $this->get_cost_by_store_location_setting() === false
			) {
			return;
		}
		...
		$store_origin_name = wc_get_order( $order_id )->get_meta( '_lpac_order__origin_store_name' );
		...
		?>
			<br/>
			<h2 class='woocommerce-order-details__title'><?php echo esc_html( $store_origin_name_label ); ?></h2>
			<p class='lpac_order_details_deliver_from_text' style='font-size: 20px; font-weight: bold'><?php echo esc_html( $store_origin_name ); ?></p>
		<?php
	}
	...
	public function lpac_output_map_on_order_details_page() {
		...
		?>
		<section class="s-order-delivery">
			<div class="container container-order-delivery">
				<h2 class="order-delivery__title">Delivery</h2>
				<div id="lpac-map-container" class='woocommerce-shipping-fields__field-wrapper'>
					<?php do_action( 'lpac_before_order_details_map', '', $user_id ); ?>
					<div class='lpac-map'></div>
					<?php do_action( 'lpac_after_order_details_map', '', $user_id ); ?>
				</div>
				<div class="order-delivery__info-block">
					<h3 class="order-delivery__info-title">Delivery & Order information</h3>
					<div class="order-delivery__info-wrapper">
						<div class="order-delivery__info-wrap">
							<span class="order-delivery__info-wrap--title">Order Date:</span>
							<span class="order-delivery__info-wrap--subtitle"><?php echo wc_format_datetime( $order->get_date_created() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
						</div>
						<div class="order-delivery__info-wrap">
							<span class="order-delivery__info-wrap--title">Price Delivery:</span>
							<span class="order-delivery__info-wrap--subtitle">
							<?php
						...
						<div class="order-delivery__info-wrap">
							<span class="order-delivery__info-wrap--title">E-mail:</span>
							<span class="order-delivery__info-wrap--subtitle"><?php echo $order->get_billing_email(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
						</div>
						<div class="order-delivery__info-wrap">
							<span class="order-delivery__info-wrap--title">Address:</span>
							<span class="order-delivery__info-wrap--subtitle"><?php echo $order->get_address('billing')['address_1']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
						</div>
					</div>
				</div>
			</div>
	...
	public function lpac_output_map_custom_styles() {
		...
		?>
		<style>
			.lpac-map{ <?php echo esc_attr( $style ); ?> }
			@media screen and (max-width: 960px ){
				.lpac-map{ <?php echo esc_attr( $style_mobile ); ?> }
			}
		</style>
	...
	public function add_admin_checkout_notice() {
		...
		if ( empty( $api_key ) ) {
			...
			<p style=" color: #ffffff !important; font-size:14px;"><span style="font-weight: bold">Location Picker at Checkout: </span>
				<?php echo wp_kses_post( $no_api_key ); ?>
			</p>
			</div>
			<?php
		}
	...
	public function createFrontendTranslatedStrings() {
		...
		$strings = array(
			...
			'generic_last_order_address' => __( 'Previous Address', 'map-location-picker-at-checkout-for-woocommerce' ),
		);
